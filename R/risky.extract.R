risky.extract <- function(df){
  ...
  lapply(risky, function(risky){
    if(!is.null(risky)){
      ...
      riskySafesLong <- reshape::melt.data.frame(risky[risky$trial.phase=="test",c("previous.bust","previous.missedBinary",safes)], id.vars=c("previous.bust","previous.missedBinary"))
